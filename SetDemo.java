...
public class SetDemo
{
   ...
   public static <T> void setOperationsDemo(Set<T> s1, Set<T> s2, Set<T> s3)
   {
       ...
       System.out.println("(s1 - s2) u (s2 - s1) = " + (s1.diff(s2)).union(s2.diff(s1)).toString());
       System.out.println("(s1 u s2) - (s1 n s2) = " + (s1.union(s2)).diff(s1.intersect(s2)).toString());
   ...
   public static <T extends Comparable> T getMax(Set<T> s)
   {
       ...
       for(int i = 1; i < s.elements.size(); i++) {
           if(max.compareTo(s.elements.get(i)) < 0)
           {
